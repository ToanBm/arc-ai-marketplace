...
import { submitSummarization } from "@/lib/api";
...
export default function SummarizationForm({ onLog, onStart }: FormProps) {
  ...
  const { data: quote, error: quoteError } = useQuote("summarization");
  ...
  return (
    <div className="mt-4">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div>
          <label className="block text-sm text-gray-400 mb-1">Text to summarize</label>
          <textarea
            {...register("text")}
            rows={6}
        ...
        {!configError && quoteError && (
          <p className="text-xs text-yellow-400">No summarization providers online. Start agent servers (E / H).</p>
        )}
